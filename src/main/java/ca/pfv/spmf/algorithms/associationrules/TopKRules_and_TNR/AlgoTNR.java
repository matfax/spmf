...
//		}
    ...
    private void cleanResult() {
        ...
        RuleG minimum = kRules.minimum();
        if (minimum != null) {
            minsuppRelative = minimum.getAbsoluteSupport();
        }
    }
     ...
     */
    public void writeResultTofile(String path) throws IOException, URISyntaxException {
        // Prepare the file
        BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(this.getClass().getClassLoader().getResource(path).toURI()))));

        // for each rule
        for (RuleG rule : kRules) {
            // Write the rule
            String buffer = "";
