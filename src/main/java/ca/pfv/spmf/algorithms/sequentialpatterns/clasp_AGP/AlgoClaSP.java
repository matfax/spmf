...
public class AlgoClaSP {
     ...
     * able to access to all the frequent ca.pfv.spmf.patterns generated by ClaSP
     */
...
//        
    ...
    protected void claSP(SequenceDatabase database, long minSupAbsolute, boolean keepPatterns, boolean verbose, boolean findClosedPatterns, boolean executePruningMethods) {
        //We get the initial trie whose children are the frequent 1-ca.pfv.spmf.patterns
        ...
        if (verbose) {
            //System.out.println("ClaSP: The algorithm takes " + (mainMethodEnd - mainMethodStart) / 1000 + " seconds and finds " + numberOfFrequentPatterns + " ca.pfv.spmf.patterns");
        	System.out.println("ClaSP: The algorithm takes " + (mainMethodEnd - mainMethodStart) + " ms and finds " + numberOfFrequentPatterns + "ca/pfv/spmf/patterns");
        }
        //If the we are interested in closed ca.pfv.spmf.patterns, we dfsPruning the post-processing step
...
//        	ShowTrie.showTree(this.getFrequentAtomsTrie());
            ...
            if (verbose) {
                System.out.println("ClaSP:The post-processing algorithm to remove the non-Closed ca.pfv.spmf.patterns takes " + (postProcessingEnd - postProcessingStart) / 1000 + " seconds and finds " + numberOfFrequentClosedPatterns + " Closed ca.pfv.spmf.patterns");
            }
        }else{
            if(keepPatterns){
                List<Entry<Pattern, Trie>> outputPatternsFromMainMethod = frequentAtomsTrie.preorderTraversal(null);
